...
class _FakeSchedulingRepository implements SchedulingRepository {
  ...
  Future<Result<ScheduleValidation>> validate(ScheduleRequest request) async =>
      Success(
        ScheduleValidation(valid: valid, message: valid ? null : 'Not allowed'),
      );
  @override
...
void main() {
  group('SchedulingMapper', () {
    ...
    test('maps a slot dto', () {
      final slot = mapper.toEntity(
        const TimeSlotDto(start: '08:00', end: '12:00'),
      );
    ...
    test('builds a request body with an ISO date', () {
      final body = mapper.requestToBody(
        ScheduleRequest(
          serviceId: 's1',
  ...
  group('SchedulingNotifier', () {
    ProviderContainer makeContainer(bool valid) => ProviderContainer(
      overrides: [
        schedulingRepositoryProvider.overrideWithValue(
          _FakeSchedulingRepository(valid),
        ),
      ],
    );
    ...
    test('confirm succeeds when valid', () async {
      ...
      expect(ok, isTrue);
      expect(
        container.read(schedulingProvider).status,
