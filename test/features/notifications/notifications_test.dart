  ...
  Future<Result<List<AffiliateNotification>>> list(String username) async =>
      const Success([
        AffiliateNotification(
          id: 'n1',
          type: NotificationType.providerAssignment,
          message: 'hi',
        ),
        AffiliateNotification(
          id: 'n2',
          type: NotificationType.canceledAssistance,
          read: true,
        ),
      ]);
  @override
...
void main() {
  ...
  group('NotificationsMapper', () {
    ...
    test('unknown type falls back to unknown', () {
      final n = mapper.toEntity(
        const NotificationDto(id: '2', type: 'WHATEVER'),
      );
  ...
  group('NotificationsNotifier', () {
    ProviderContainer makeContainer(int unread) => ProviderContainer(
      overrides: [
        cachedSessionProvider.overrideWith((_) => session),
        notificationsRepositoryProvider.overrideWithValue(
          _FakeRepository(unread),
        ),
      ],
    );
    ...
    test('failure when no session', () async {
      final container = ProviderContainer(
        overrides: [
          cachedSessionProvider.overrideWith(
            (_) => const SessionData(accessToken: 'tok'),
          ),
