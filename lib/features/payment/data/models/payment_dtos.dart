/// DTOs + mapper for payment (AFILIADO `planes`, `detalle_servicio_app/`,
...
class ShopServiceDto {
  const ShopServiceDto({this.id, this.name, this.price, this.currency, this.description});
  final String? id;
  final String? name;
  final double? price;
  final String? currency;
...
class PurchaseDto {
  const PurchaseDto({this.id, this.itemType, this.itemId, this.name, this.price, this.quantity});
  final String? id;
...
class PaymentResultDto {
  const PaymentResultDto({this.success, this.paymentUrl, this.referenceId, this.message});
  final bool? success;
  final String? paymentUrl;
  final String? referenceId;
  final String? message;
  factory PaymentResultDto.fromJson(Map<String, dynamic> json) => PaymentResultDto(
        success: json['success'] is bool
            ? json['success'] as bool
            : json['redirect_url'] != null,
        paymentUrl: json['redirect_url']?.toString() ?? json['payment_url']?.toString(),
  ...
  static PaymentResultDto? tryParse(dynamic body) {
    ...
    if (body is String) {
      ...
      if (decoded is Map<String, dynamic>) return PaymentResultDto.fromJson(decoded);
    }
    return null;
  }
}
...
class PaymentMapper {
  ...
  Purchase toPurchase(PurchaseDto dto) => Purchase(
        id: dto.id ?? '',
        itemType: (dto.itemType == 'service') ? PaymentItemType.service : PaymentItemType.plan,
        itemId: dto.itemId ?? '',
        name: dto.name ?? '',
        price: dto.price ?? 0,
