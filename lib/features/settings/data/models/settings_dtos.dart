/// DTOs + mapper for settings (AFILIADO `ResponseApplicationSetting`).
library;
...
class AppConfigurationDto {
  ...
  factory AppConfigurationDto.fromJson(Map<String, dynamic> json) => AppConfigurationDto(
        primaryColor: json['primaryColor']?.toString() ?? json['primary_color']?.toString(),
        accentColor: json['accentColor']?.toString() ?? json['accent_color']?.toString(),
        ...
        displayItemBeneficiaries: json['displayItemBeneficiaries'] is bool
            ...
            : null,
        displayItemVehicles:
            json['displayItemVehicles'] is bool ? json['displayItemVehicles'] as bool : null,
        displayShoppingList:
            json['displayShoppingList'] is bool ? json['displayShoppingList'] as bool : null,
        displayItemProfile:
            json['displayItemProfile'] is bool ? json['displayItemProfile'] as bool : null,
        maxInactivityMinutes:
            json['maxInactivityMinutes'] is int ? json['maxInactivityMinutes'] as int : null,
      );
  ...
  static AppConfigurationDto? tryParse(dynamic body) {
    ...
    if (body is String) {
      ...
      if (decoded is Map<String, dynamic>) return AppConfigurationDto.fromJson(decoded);
    }
    return null;
  }
}
...
class SettingsMapper {
  ...
  int? _parseColor(String? raw) {
    ...
    if (!cleaned.startsWith('0x')) {
      // Normalize to ARGB: a 6-digit RGB becomes 0xFFRRGGBB (full alpha).
